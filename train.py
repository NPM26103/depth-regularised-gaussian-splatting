...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if depth.ndim == 2:
            ...
            Ls = torch.tensor(0.0, device="cuda")  # hoặc 0.0 nếu không dùng GPU

        if depth.ndim == 2 and gt_depth.shape == depth.shape:
            Ld = l1_loss(depth, gt_depth)
        else:
            Ld = torch.tensor(0.0, device="cuda")  # hoặc bỏ hoàn toàn loss này nếu không có ảnh depth
